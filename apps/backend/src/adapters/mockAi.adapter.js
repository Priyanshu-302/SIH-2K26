 ...
 */
export async function* mockAgentStream({ query, sessionId, history, jurisdiction = 'national', options } = {}) {
  ...
  // 1. Simulate reasoning/thinking phase (customized by jurisdiction)
  const reasoningTokens = jurisdiction === 'international' ? [
    'System: Initiating International IPR & Cross-Border Treaties Search...\n',
    'Analyzing WIPO GRATK Treaty (2024) Article 3 mandatory disclosure mandates...\n',
    'Checking CBD & Nagoya Protocol ABS Clearing-House (PIC / MAT / IRCC)...\n',
    'Evaluating PCT 30-Month Filing Route and Destination Market Regimes (US FDA / EU EMA)...\n\n',
    '**Ayur-IP – International Patent & Export Regulatory Assessment:**\n\n'
  ] : [
  ...
  const reasoningDelay = process.env.NODE_ENV === 'test' ? 5 : 80;
  for (const token of reasoningTokens) {
    yield { type: 'token', data: token };
    await new Promise((resolve) => setTimeout(resolve, reasoningDelay));
  }

  let mockCitations = [];
  let responseText = '';

  // 0. International & Export Regime Handling
  if (jurisdiction === 'international') {
    mockCitations = [
      {
        id: 'cit-wipo-gratk-art3',
        source: 'WIPO GRATK Treaty (2024)',
        section: 'Article 3',
        snippet: 'Patent applicants must disclose the country of origin of genetic resources and/or the Indigenous community that provided associated traditional knowledge in foreign patent applications.',
        confidence: 'high',
        url: 'https://www.wipo.int/wipolex/en/treaties/ShowPDF/TRT/GRATK/001'
      },
      {
        id: 'cit-nagoya-ircc',
        source: 'CBD Nagoya Protocol',
        section: 'Article 5, 6 & 17',
        snippet: 'Access to genetic resources requires Prior Informed Consent (PIC) and Mutually Agreed Terms (MAT). Internationally Recognized Certificate of Compliance (IRCC) generated by NBA India.',
        confidence: 'high',
      ...
      },
      {
        ...
        section: 'Rule 51bis & Indian Patents Act Section 39',
        snippet: 'Mandatory Foreign Filing Permission (Form 25) before filing abroad. PCT 30-month national phase entry with biological source declarations.',
        confidence: 'high',
        url: 'https://www.wipo.int/pct/'
      },
      {
        id: 'cit-us-fda-dshea',
        source: 'US FDA DSHEA & Botanical Guidance',
        section: '21 U.S.C. § 343(r)(6)',
        snippet: 'Structure/function claims allowed; disease treatment/cure claims strictly barred without FDA new drug approval (NDA). Complete botanical raw material traceability required.',
        confidence: 'high',
        url: 'https://www.fda.gov'
      },
      {
        id: 'cit-eu-thmpd',
        source: 'EU Directive 2004/24/EC (THMPD)',
        section: 'Traditional Use Requirement',
        snippet: 'Simplified registration requires proof of at least 30 years of traditional medicinal use, of which at least 15 years must be within the EU/EEA.',
        confidence: 'high',
        url: 'https://www.ema.europa.eu'
      }
    ];

    responseText = `### 1. Mandatory Source & Origin Disclosure (WIPO GRATK Treaty 2024)
Under **Article 3 of the WIPO Treaty on Intellectual Property, Genetic Resources and Associated Traditional Knowledge (adopted May 24, 2024)**, when filing patent applications in foreign member states based on Indian botanicals, your research team is legally obligated to disclose:
- **India as the Country of Origin** of the botanical biological resources.
- The classical Ayurvedic treatises (*Charaka Samhita*, *Sushruta Samhita*) or the **Traditional Knowledge Digital Library (TKDL)** as the **Source of Associated Traditional Knowledge**.
- Under **Article 4**, this mandatory disclosure applies prospectively without retroactive invalidation of prior granted patents. Under **Article 7**, foreign patent offices cannot revoke a granted patent solely for post-grant disclosure errors unless intentional fraudulent misrepresentation is established.

---

### 2. Cross-Border Access & Benefit-Sharing (Nagoya Protocol & Indian Section 39)
- **Indian Patents Act Section 39 (Criminal Penalty Prerequisite):** Before filing any foreign patent application or PCT international request, you must obtain written **Foreign Filing Permission (FFP / Form 25)** from the Indian Patent Office, unless 6 weeks have elapsed since your Indian priority filing.
- **Nagoya Protocol (CBD) Compliance:** Sourcing biological material from India for international commercialization requires **Prior Informed Consent (PIC)** and **Mutually Agreed Terms (MAT)** executed with the National Biodiversity Authority (NBA). NBA approval generates an **Internationally Recognized Certificate of Compliance (IRCC)** uploaded to the global ABS Clearing-House (ABSCH), providing absolute legal immunity in foreign patent jurisdictions.

---

### 3. International Patent Prosecution Strategy (The 30-Month PCT Route)
- **Filing Pathway:** File an Indian priority application (Month 0), followed by an **international PCT application within 12 months**. This grants a **30-month window** to evaluate commercial viability before entering the national phase in the US (USPTO), Europe (EPO), or Japan (JPO).
- **Overcoming Foreign Natural Product Exclusions:**
  - In the United States, raw botanicals face severe 35 U.S.C. 101 subject matter eligibility bars (*Alice / Mayo* doctrine).
  - In Europe, raw botanical mixtures are excluded under EPC Article 52(2)(a) as mere discoveries.
  - **The Winning Strategy:** Claim **standardized synergistic bioactive ratios** (e.g. 3:1 standardized withanolide-bacoside complex demonstrating >30% non-obvious efficacy index) or **proprietary phytosomal/liposomal nano-carriers** rather than crude plant extracts.
  - If microbial biotransformation or fermentation (*Sandhana Kalpana / Asava-Arishta*) is involved, deposit the specific strain with an International Depositary Authority (**MTCC Chandigarh**) under **Article 3 of the Budapest Treaty**.

---

### 4. Destination Market Regulatory Classification (US vs EU)
- **United States (US FDA):**
  - *Dietary Supplement Route (DSHEA 1994, 21 U.S.C. § 343(r)(6)):* You may market the tonic with **structure/function claims** (*"promotes cognitive focus and memory retention"*). You **must not make disease claims** (*"prevents Alzheimer's or dementia"*), which instantly misbrands the product as an unapproved new drug.
  - *Prescription Botanical Route (CDER Guidance 2016):* If claiming clinical disease treatment, you must file an Investigational New Drug (IND) application with multi-batch chromatographic fingerprinting (HPLC/LC-MS) for batch-to-batch consistency.
- **European Union (EU EMA):**
  - *Directive 2004/24/EC (THMPD):* Simplified herbal registration requires proof of **30 years of traditional medicinal use, with at least 15 years within the EU/EEA**. Because many Indian classical herbs lack 15 years of continuous European market presence, innovators frequently export via the **EU Food Supplement Directive (2002/46/EC)** with EFSA contaminant compliance.

---

### 5. Executive Export Recommendation & Action Items
1. **File Indian Priority First (Month 0):** Establish priority in India and file Form 25 for Section 39 clearance.
2. **Submit Form 1 to the NBA:** Concurrently apply for cross-border ABS clearance to secure your global IRCC certificate.
3. **Trigger PCT Application at Month 12:** Maximize the 30-month runway before committing to expensive foreign national phase fees.
4. **Protect Global Brand Identity via WIPO Madrid Protocol:** File Form MM2 through the Indian Trade Marks Registry to protect your brand name across **Class 5 (Medicines)** and **Class 30 (Wellness Infusions)** in target export countries.`;

  // 1. National Biodiversity Authority (NBA) & Section 6 of BD Act
  } else if (normalizedQuery.includes('biodiversity') || normalizedQuery.includes('nba') || (normalizedQuery.includes('section 6') && !normalizedQuery.includes('saffron'))) {
    mockCitations = [
      {
        id: 'cit-bd-act-sec6',
...
To achieve patentability, you must demonstrate a non-obvious synergistic composition with enhanced therapeutic efficacy under Section 3(e) or 3(d).`;
  ...
  const citationDelay = process.env.NODE_ENV === 'test' ? 5 : 80;
  await new Promise((resolve) => setTimeout(resolve, citationDelay));

  // Yield assessment response tokens with natural typing cadence
  const tokenDelay = process.env.NODE_ENV === 'test' ? 1 : 16;
  const chunkSize = process.env.NODE_ENV === 'test' ? 6 : 2;
